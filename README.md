...
- **Per-repo comment keying** ([#24](https://github.com/saurabh-hirani/review.nvim/pull/24)) — comments are keyed by the git root of each file, not by nvim's working directory. Open nvim from anywhere, review files across several repos in one session, and each repo keeps its own per-branch review file. `:Review quickfix`, `:Review list`, and export all scope to the repo of the file in the current buffer, and marks render for that repo — so jumps, exports, and marks resolve to the right files regardless of cwd. Previously the cwd's git root decided everything, so reviewing a file from a sibling repo stored the comment under the wrong repo and its jumps, marks, and export broke.
  - **Breaking change** — comments are now stored under a key derived from each file's own git root, so review files written by earlier versions (keyed by the cwd git root) no longer match and their comments will not appear. There is no automatic migration. Delete the old review files from `$XDG_DATA_HOME/nvim/review/` (on most setups `~/.local/share/nvim/review/`; run `:echo stdpath("data")` in nvim to confirm the path) and start fresh, or move each file to the name derived from the reviewed repo's git root if you want to keep old comments.
